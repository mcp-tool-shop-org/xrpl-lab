...
每个模块教授一项 XRPL 技能，并生成可验证的成果：交易 ID、已签名的收据或诊断报告。无需账户、无需冗余信息、无需云服务——只需掌握技能和获得凭证。
...
共 28 个模块，分为十个主题：基础知识、NFT、令牌、支付、身份验证、去中心化交易所 (DEX)、储备金、审计、自动做市商 (AMM) 和综合项目。
先决条件明确——命令行界面和代码检查工具会强制执行这些条件。

“#”列与 `xrpl-lab list` 显示的顺序相匹配（规范的主题顺序）。
...
| 5 | NFT 铸造 101：您的第一个游戏资产 | nft | 测试网络 | — | txid、报告 |
| 6 | NFT 市场 101：交易具有强制版税的资产 | nft | 测试网络 | — | txid、报告 |
| 7 | 动态 NFT 101：一个可以升级的游戏物品 | nft | 测试网络 | — | txid、报告 |
| 8 | MPT 发行 101：一次性完成的游戏货币发行 | 令牌 | 测试网络 | — | txid、报告 |
| 9 | MPT 分发 101：将货币分发给玩家 | 令牌 | 测试网络 | MPT 发行 101：一次性完成的游戏货币发行 | txid、报告 |
| 10 | 令牌冻结 101：发行者的暂停按钮 | 令牌 | 测试网络 | — | txid、报告 |
| 11 | 召回 101：发行者的召回机制 | 令牌 | 测试网络 | — | txid、报告 |
| 12 | 托管 101：时间锁定的 XRP | 支付 | 测试网络 | — | txid、报告 |
| 13 | 托管完成 101：释放锁定的 XRP | 支付 | 测试网络 | 托管 101：时间锁定的 XRP | txid、报告 |
| 14 | 令牌托管 (XLS-85)：锁定 IOU，而不仅仅是 XRP | 支付 | 测试网络 | 信任线 101：已发行货币作为关系，托管 101：时间锁定的 XRP | txid、报告 |
| 15 | 支付通道 101：一次签名，多次结算 | 支付 | 测试网络 | — | txid、报告 |
| 16 | 已交付金额：部分支付漏洞 | 支付 | 测试网络 | 信任线 101：已发行货币作为关系 | txid、报告 |
| 17 | DID 101：链上身份 | 身份验证 | 测试网络 | — | txid、报告 |
| 18 | 凭据 101 (XLS-70)：链上 KYC 和年龄证明 | 身份验证 | 测试网络 | DID 101：链上身份 | txid、报告 |
| 19 | 权限域和受限 DEX (XLS-80/81)：符合法规、基于凭据的交易 | 身份验证 | 测试网络 | 凭据 101 (XLS-70)：链上 KYC 和年龄证明 | txid、报告 |
| 20 | DEX 解读：报价、订单簿和取消 | dex | 测试网络 | 信任线 101：已发行货币作为关系 | txid、报告 |
| 21 | DEX 市场做市商 101：从订单簿中赚取价差 | dex | 测试网络 | DEX 解读：报价、订单簿和取消 | txid、报告 |
| 22 | DEX 库存保障措施：避免出现单边风险 | dex | 测试网络 | DEX 市场做市商 101：从订单簿中赚取价差 | txid、报告 |
| 23 | 储备金 101：您的 XRP“去向” | reserves | 测试网络 | 信任线 101：已发行货币作为关系 | txid、报告 |
| 24 | 账户维护：释放储备金并清理对象 | reserves | 测试网络 | 储备金 101：您的 XRP“去向” | txid、报告 |
| 25 | 审计模式：大规模验证收据 | audit | 测试网络 | 收据解读 | 报告、audit_pack |
| 26 | AMM 流动性 101：提供流动性和赚取费用 | amm | dry-run | 信任线 101：已发行货币作为关系 | txid、报告 |
| 27 | DEX 与 AMM 风险解读：比较交易策略 | amm | dry-run | DEX 市场做市商 101：从订单簿中赚取价差，AMM 流动性 101：提供流动性和赚取费用 | txid、报告 |
| 28 | 综合项目：在 XRPL 上构建一个最小的游戏经济系统 | capstone | 测试网络 | MPT 发行 101：一次性完成的游戏货币发行，NFT 铸造 101：您的第一个游戏资产，托管 101：时间锁定的 XRP，审计模式：大规模验证收据 | txid、报告、audit_pack |
...
所有命令都支持 `--dry-run` 选项，以便在适用时进入离线模式。
...
一位助教可以从支持包中诊断任何学习者的所有问题，而无需重现整个会话。不包含任何秘密信息。
...
**XRPL Lab 访问的数据：**
- 钱包种子（以纯文本 JSON 形式存储在 `~/.xrpl-lab/wallet.json` 中，受 0o600 文件权限和 0o700 父目录保护——未加密）
- 模块进度和交易 ID（存储在 `~/.xrpl-lab/state.json` 中，通过 tmp + 重命名进行原子写入）
- XRPL 测试网 RPC（公共端点，提交前本地签名交易）
...
- 证明包、证书、报告或支持文件中绝不会包含任何敏感信息。

**权限和存储层级：**
- 主目录 `~/.xrpl-lab/`——私有密钥层级，0o700 目录 + 0o600 钱包文件。存储钱包种子、日志文件和审计包。
...
- 所有错误帧都发出结构化的信封（`code`、`message`、`hint`、`severity`、`icon_hint`），不会泄露路径或内部状态。
