...
包含十个主题的24个模块：基础知识、NFT、令牌、支付、身份验证、去中心化交易所（DEX）、储备金、审计、自动做市商（AMM）和综合项目。
...
| # | 模块 | 主题 | 模式 | 先决条件 | 输出结果 |
|---|--------|-------|------|---------------|----------|
| 1 | 收据解读 | 基础知识 | 测试网络 | — | 交易 ID (txid)，报告 |
| 2 | 失败解读 | 基础知识 | 测试网络 | 收据解读 | 交易 ID (txid)，报告 |
| 3 | 信任线 101：已发行货币作为关系 | 基础知识 | 测试网络 | — | 交易 ID (txid)，报告 |
| 4 | 调试信任线 | 基础知识 | 测试网络 | 信任线 101：已发行货币作为关系 | 交易 ID (txid)，报告 |
| 5 | NFT 铸造 101：您的第一个游戏资产 | NFT | 测试网络 | — | 交易 ID (txid)，报告 |
| 6 | NFT 市场 101：交易具有强制版税的资产 | NFT | 测试网络 | — | 交易 ID (txid)，报告 |
| 7 | 动态 NFT 101：一个可以升级的游戏物品 | NFT | 测试网络 | — | 交易 ID (txid)，报告 |
| 8 | 多用途令牌 (MPT) 发行 101：一次性发行游戏货币 | 令牌 | 测试网络 | — | 交易 ID (txid)，报告 |
| 9 | MPT 分发 101：将货币发送给玩家 | 令牌 | 测试网络 | 多用途令牌 (MPT) 发行 101：一次性发行游戏货币 | 交易 ID (txid)，报告 |
| 10 | 令牌冻结 101：发行者的暂停按钮 | 令牌 | 测试网络 | — | 交易 ID (txid)，报告 |
| 11 | 召回 101：发行者的召回机制 | 令牌 | 测试网络 | — | 交易 ID (txid)，报告 |
| 12 | 托管 101：时间锁定的 XRP | 支付 | 测试网络 | — | 交易 ID (txid)，报告 |
| 13 | 托管完成 101：释放锁定的 XRP | 支付 | 测试网络 | 托管 101：时间锁定的 XRP | 交易 ID (txid)，报告 |
| 14 | 支付通道 101：一次签名，多次结算 | 支付 | 测试网络 | — | 交易 ID (txid)，报告 |
| 15 | DID 101：链上身份验证 | 身份验证 | 测试网络 | — | 交易 ID (txid)，报告 |
| 16 | DEX 知识：报价、订单簿和取消 | 去中心化交易所 | 测试网络 | 信任线 101：已发行货币作为关系 | 交易 ID (txid)，报告 |
| 17 | DEX 市场做市商 101：从订单簿中赚取价差 | 去中心化交易所 | 测试网络 | DEX 知识：报价、订单簿和取消 | 交易 ID (txid)，报告 |
| 18 | DEX 库存保障措施：避免出现严重失衡 | 去中心化交易所 | 测试网络 | DEX 市场做市商 101：从订单簿中赚取价差 | 交易 ID (txid)，报告 |
| 19 | 储备金 101：您的 XRP “转移到”哪里 | 储备金 | 测试网络 | 信任线 101：已发行货币作为关系 | 交易 ID (txid)，报告 |
| 20 | 账户维护：释放储备金并清理对象 | 储备金 | 测试网络 | 储备金 101：您的 XRP “转移到”哪里 | 交易 ID (txid)，报告 |
| 21 | 审计模式：大规模验证收据 | 审计 | 测试网络 | 收据解读 | 报告，审计包 |
| 22 | AMM 流动性 101：提供流动性和赚取费用 | 自动做市商 | 模拟运行 | 信任线 101：已发行货币作为关系 | 交易 ID (txid)，报告 |
| 23 | DEX 与 AMM 风险知识：比较交易策略 | 自动做市商 | 模拟运行 | DEX 市场做市商 101：从订单簿中赚取价差，AMM 流动性 101：提供流动性和赚取费用 | 交易 ID (txid)，报告 |
| 24 | 综合项目：在 XRPL 上构建一个最小的游戏经济系统 | 综合项目 | 测试网络 | MPT 发行 101：一次性发行游戏货币，NFT 铸造 101：您的第一个游戏资产，托管 101：时间锁定的 XRP，审计模式：大规模验证收据 | 交易 ID (txid)，报告，审计包 |
<!-- END curriculum:auto readme-table -->

“输出结果”列显示了每个模块生成的内容（`txid`、`report`、`audit_pack`）；请参阅[手册](https://mcp-tool-shop-org.github.io/xrpl-lab/handbook/modules/)中每个模块的页面，以了解完整的技能演练以及您在链上证明的内容。
