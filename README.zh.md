...
引导式启动程序将指导您完成钱包设置、充值以及第一个模块的学习。
...
| 3 | 信任线基础知识 | 初级 | 创建发行方，设置信任线，发行令牌 | 信任线 + 令牌余额 |
...
| 10 | 去中心化交易所 (DEX) 做市基础知识 | 高级 | 买入/卖出订单，持仓快照，清理 | 策略交易 ID + 清理报告 |
| 11 | 库存管理 | 高级 | 基于阈值的报价，仅限单向下单 | 库存检查 + 保护交易 ID |
| 12 | 去中心化交易所 (DEX) 与自动做市商 (AMM) 风险知识 | 高级 | 去中心化交易所 (DEX) 和自动做市商 (AMM) 生命周期对比 | 对比报告 + 交易记录 |
...
**XRPL Lab 访问的数据：**
- 钱包助记词（以受限文件权限存储在 `~/.xrpl-lab/wallet.json` 中）
- 模块进度和交易 ID（存储在 `~/.xrpl-lab/state.json` 中）
- XRPL 测试网 RPC（公共端点，交易在本地签名后提交）
- 测试网水龙头（公共 HTTP，仅发送您的地址）

**XRPL Lab 不会访问的数据：**
- 不支持主网，仅支持测试网。
- 不收集任何形式的遥测数据、分析数据或远程访问。
