...
**Auth model:** the entire `/api/runs` surface inherits the same CORS gate as the rest of the HTTP API — `server.py` restricts to localhost. There is no token auth in v1.7.0; if you're exposing this surface beyond loopback (which the workshop threat model does not recommend), front it with your own auth layer.
