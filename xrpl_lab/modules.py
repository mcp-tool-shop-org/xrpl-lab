...
def render_module_skeleton(
    *,
    ...
    body = (
        ...
        "     Pick ONE of the actions below and add it to this step as an\n"
        "     HTML comment, in the same form as Step 1 above:\n"
        "       action: ensure_funded\n"
        "       action: submit_payment destination=ADDRESS amount=10\n"
        "       action: set_trust_line currency=LAB limit=1000\n"
        "     One action per step - a second action under the same heading\n"
        "     never runs, and the linter rejects it.\n"
