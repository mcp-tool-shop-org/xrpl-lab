...
def render_module_skeleton(
    *,
    ...
    body = (
        ...
        "     Replace the placeholder action below with the real one.\n"
        "     Examples:\n"
        "       action: ensure_funded\n"
        "       action: submit_payment destination=ADDRESS amount=10\n"
        "       action: set_trust_line currency=LAB limit=1000\n"
        "     Exactly one action per Step heading — zero actions (or a\n"
        "     typo'd comment that fails to match) and a second action\n"
        "     under the same heading are both lint errors. Use\n"
        "     <!-- narrative-only --> only for intentionally actionless\n"
        "     Steps; ## Checkpoint: wrap-ups need no marker.\n"
        ...
        "     Replace narrative-only with a verify_* action when this\n"
        "     step should hit the ledger; keep the marker when it is\n"
