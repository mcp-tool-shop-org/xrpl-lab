...
checks:
  ...
  - "DID deleted (txid produced)"
  - "DID gone from the ledger — identity revoked, reserve freed"
---
...
## Step 5: Revoke the DID

Identity hygiene means knowing how to **undo** an identity, not just create one. `DIDDelete`
removes the on-ledger DID object — the account no longer claims that identifier — and returns the
owner reserve the DID was holding. This mirrors how *Account Hygiene* closes the trust-line story:
every object you create, you should know how to remove.

<!-- action: delete_did -->

## Step 6: Verify it's gone

Read `account_objects` once more. The DID object should be absent — the identity is revoked and
the reserve it held is freed back to spendable.

<!-- action: verify_did_deleted -->

## Checkpoint: What you proved

You anchored a decentralized identifier to your account, verified it, then revoked it on-ledger:
...
4. **DIDDelete** — revoked the identity and freed its owner reserve
...
- **One DID per account** — `DIDSet` creates or updates it; `DIDDelete` (Step 5) removes it and frees the reserve.
